...
const Header = () => {
  ...
  const url='https://www.seekpng.com/png/full/245-2454602_tanni-chand-default-user-image-png.png';
    return (
        ...
        {
            user? <div className="p-2"> <Link to='/myitems' className=' text-decoration-none p-4'>My Items</Link> <Link to='/' className='p-4 mt-2 text-decoration-none'>Manage Items</Link> <Link to='/' className='p-4 mt-2 text-decoration-none'>Add Item</Link> <span className="text-black ms-5">{user?.displayName}  </span> <span><img src={user?.photoURL || url}style ={{height:"60px", width:"60px", borderRadius:"20px"}}alt="user"></img></span><button className='btn btn-link text-black text-decoration-none ms-5 ' onClick={handleSignOut}>  sign out</button> </div>  :
